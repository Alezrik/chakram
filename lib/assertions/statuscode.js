...
module.exports = function (chai, utils) {
    ...
    utils.addMethod(chai.Assertion.prototype, 'status', function (status) {
        ...
        this.assert(
            ...
            ' call: method: '+ this._obj.request.method + ' path: ' + this._obj.request.uri.pathname  +' status_code' + this._obj.response.status_code + ' respTime:' + this_obj.response.headers.x_response_time + ' cf-request-id:' + this._obj.response.headers.cf_request_id ,
            'expected status code ' + respStatus + ' to equal ' + status +
            ' call: method: '+ this._obj.request.method + ' path: ' + this._obj.request.uri.pathname  +' status_code' + this._obj.response.status_code + ' respTime:' + this_obj.response.headers.x_response_time + ' cf-request-id:' + this._obj.response.headers.cf_request_id 
        );
    });
